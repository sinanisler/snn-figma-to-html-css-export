...
describe('AI rebuild', () => {
	const hero = () =>
		node({
			...
			width: 1440,
			height: 600,
			children: [text('H1', 'Build faster websites', { x: 120, y: 200 }), text('Intro', 'Everything you need', { x: 120, y: 300 })],
		});
	const features = () =>
		node({
			name: 'Features',
			...
			width: 1440,
			height: 400,
			children: [text('H2', 'Features list here', { x: 120, y: 40 }), node({ name: 'Grid', x: 120, y: 120, children: [text('Item', 'Fast')] })],
		});
	const page = () =>
		build(
			result([
				node({
					...
					fills: [{ type: 'SOLID', color: white, opacity: 1 }],
					children: [
						node({ name: 'Background', type: 'RECTANGLE', width: 1440, height: 1000, fills: [{ type: 'SOLID', color: { r: 0.9, g: 0.9, b: 1, a: 1 }, opacity: 1 }] }),
						features(),
						hero(),
					],
				}),
	...
	it('splits a page into ordered sections inside a flowing wrapper', () => {
		const doc = page();
		const [plan] = planPages(doc, { assets: new Map() });
		expect(plan.roots).toHaveLength(1);
		const wrapper = plan.roots[0];
		expect(wrapper.kind).toBe('wrapper');
		if (wrapper.kind !== 'wrapper') return;
		expect(wrapper.style).toMatchObject({ display: 'flex', 'flex-direction': 'column', width: '100%', 'background-color': '#e6e6ff' });
		expect(wrapper.style.height).toBeUndefined();
		expect(plan.sections.map((s) => s.label)).toEqual(['Hero', 'Features']);
		expect(plan.sections[0].html).toContain('Build faster websites');
		expect(plan.sections[0].css).toContain('.hero {');
		expect(plan.sections[0].words).toEqual(expect.arrayContaining(['build', 'faster', 'websites']));
	});

	it('always splits a page-sized frame, but keeps a small component whole', () => {
		const [plan] = planPages(page(), { assets: new Map() });
		expect(plan.sections.map((s) => s.label)).toEqual(['Hero', 'Features']);
		const [card] = planPages(build(result([node({ name: 'Card', width: 380, height: 220, children: [node({ name: 'Top', children: [text('T', 'Title')] }), node({ name: 'Bottom', children: [text('B', 'Body')] })] })])), { assets: new Map() });
		expect(card.sections.map((s) => s.label)).toEqual(['Card']);
	});

	it('builds prompts with the section, prefix and optional screenshot', () => {
		const doc = page();
		const [plan] = planPages(doc, { assets: new Map() });
		const messages = sectionMessages(plan.sections[1], { doc, plan, styling: 'tailwind', instructions: 'Use a 1200px container', screenshot: 'data:image/jpeg;base64,AAA' });
		expect(messages[0].content).toContain('Tailwind CSS v4');
	...
	it('parses model answers and strips unsafe parts', () => {
		const parsed = parseSectionResponse(
			'Sure!\n```html\n<section class="hero" onclick="x()"><h1>Hi</h1><style>.hero{color:red}</style><script>alert(1)</script></section>\n```\n```css\n.hero-title { font-size: 2rem; }\n```',
		);
		expect(parsed.html).toBe('<section class="hero"><h1>Hi</h1></section>');
		expect(parsed.css).toContain('.hero-title { font-size: 2rem; }');
		expect(parsed.css).toContain('.hero{color:red}');
		expect(() => parseSectionResponse('I cannot do that.')).toThrow();
	});

	it('measures how much text survived', () => {
		const doc = page();
		const [plan] = planPages(doc, { assets: new Map() });
		expect(textCoverage(plan.sections[0], '<h1>Build faster websites</h1><p>Everything you need</p>')).toBe(1);
		expect(textCoverage(plan.sections[0], '<h1>Build websites</h1>')).toBeLessThan(0.8);
	});

	it('stitches results and falls back to the standard output for missing sections', () => {
		const doc = page();
		const plans = planPages(doc, { assets: new Map() });
		const results = new Map([[plans[0].sections[0].key, { html: '<section class="hero"><h1>Build faster websites</h1></section>', css: '.hero { padding: 4rem; }' }]]);
		const assembled = assemblePages({ doc, plans, results, assets: new Map(), styling: 'css' });
		const body = assembled.pages[0].body;
		expect(body).toContain('<div class="landing">');
		expect(body.indexOf('class="hero"')).toBeLessThan(body.indexOf('class="features"'));
		expect(assembled.css).toContain('.hero { padding: 4rem; }');
		expect(assembled.css).toContain('/* Features (standard output) */');
		expect(assembled.css).not.toMatch(/\.features \{[^}]*position: absolute/);

		const react = aiFiles(doc, assembled, { format: 'react', assets: new Map(), assetMode: 'files', googleFonts: false, inlineSvg: false });
		const jsx = react.find((f) => f.path === 'src/pages/Landing.jsx')!.content;
		expect(jsx).toContain('<div className="landing">');
		expect(jsx).toContain("import '../styles.css';");
	});

	it('converts HTML to JSX', () => {
		expect(
			htmlToJsx('<label for="e" class="a" style="font-size: 12px; --x: 1">Hi {name}</label><input id="e" value="x" disabled><br><!-- note --><svg viewBox="0 0 1 1"><path stroke-width="2" /></svg>'),
		).toBe(
			'<label htmlFor="e" className="a" style={{ fontSize: "12px", "--x": "1" }}>Hi {\'{\'}name{\'}\'}</label><input id="e" defaultValue="x" disabled /><br />{/* note */}<svg viewBox="0 0 1 1"><path strokeWidth="2" /></svg>',
		);
	});

	it('rewrites asset paths and page links for each target', () => {
		const assets = new Map([['a1', { id: 'a1', name: 'logo.svg', mime: 'image/svg+xml', ext: 'svg', bytes: new TextEncoder().encode('<svg/>') }]]);
		expect(resolveAssetPaths('<img src="assets/logo.svg"> url("./assets/logo.svg") assets/other.png', assets, 'files', '/assets/')).toBe(
			'<img src="/assets/logo.svg"> url("/assets/logo.svg") assets/other.png',
		);
		expect(resolveAssetPaths('<img src="assets/logo.svg">', assets, 'inline', 'assets/')).toContain('data:image/svg+xml;base64,');
		const doc = build(result([node({ id: 'home', name: 'Home', topLevel: true }), node({ id: 'about', name: 'About', topLevel: true })]));
		expect(rewriteLinks('<a href="about.html#team">', doc, 'hash')).toBe('<a href="#/about">');
		expect(rewriteLinks('<a href="about.html#team"><a href="index.html">', doc, 'path')).toBe('<a href="/about#team"><a href="/">');
	});
});
