...
| Class naming | **Not a real decision to make up front — it's bounded by input quality.** Derive class names from Figma layer names when they're clean/usable; fall back to generic auto-numbered classes when they're not (`Rectangle 47`, `Group 12`, etc.). No BEM ceremony, no AI-based naming. | See §6.1 — you correctly identified that debating a naming *convention* is pointless when the real variable is whether the designer named their layers well at all. |
| Units | **Always `px`**, no `rem`/`em`/`%` conversion in v1. | See §6.1 for the rounding-precision follow-up. |
| Asset delivery | **SVG for vector layers, native raster format (PNG/JPG) for images. Copy-to-clipboard by default; auto-recommend a `.zip` download when assets are numerous/large.** | See §6.1 for the concrete size/count threshold approach. |
| Tooling | **Plugma** (Vite-based) over `create-figma-plugin`. | Your call, delegated to this research pass — see §6.1 for rationale. |
...
> **Correction (found during implementation, 2026-09-11):** the paragraph below is wrong about API access. The current `@figma/plugin-typings` (1.138) expose a read-only `inferredAutoLayout: InferredAutoLayoutResult | null` property on frames — documented as "what Figma uses to power Dev Mode's code snippets" and based on a heuristic. The plugin uses it: non-Auto-Layout frames with a confident inference are exported as flexbox, and only the rest fall back to absolute positioning. The custom VIPS-style heuristic below is still relevant for groups (which have no `inferredAutoLayout`) and for frames Figma can't infer.
...
5. **Config surface for the things developers actually asked for**: px rounding precision, SVG-for-vectors vs PNG-for-photos, variables→CSS-custom-properties toggle. All now resolved — see §6.1.
...
All the product-level questions are now locked in (§0). This section records the reasoning behind the last few detail decisions, plus one genuinely-open follow-up.

### 6.1 Detail decisions, resolved

**Class naming — correctly identified as a non-decision.** You're right that debating BEM-vs-slug-vs-hash is pointless: the actual constraint isn't which convention to pick, it's that a class name can only be as good as the Figma layer name behind it, and you've ruled out AI-based inference. Concrete rule for the generator: **slugify the layer name if it's usable** (not the Figma-default auto-generated kind — i.e. reject/skip patterns like `Rectangle 47`, `Group 12`, `Frame 8`, `Ellipse 3`, `Vector`, bare numbers, or names with only whitespace/symbols) and **fall back to a stable auto-numbered class** (`.el-1`, `.el-2`, scoped to the export) when it isn't. This needs zero configuration UI and zero "convention" decision — it's a pure input-quality gate, which is exactly your framing. Worth a one-line note in the plugin's own UI ("name your layers for cleaner class names") since that's the one lever the user actually controls.

**Units — always `px`.** Simpler than offering a unit-system toggle, and matches "fast export, refine later downstream" — a downstream refinement tool is a more sensible place to convert to `rem`/relative units (which usually requires establishing a root font-size convention anyway) than a deterministic 1:1 Figma exporter. The one thing still worth doing regardless of unit choice: **round to a sane precision** (e.g. 2 decimal places, or nearest integer where the fractional part is negligible) so you don't reproduce the widely-mocked `13.333333333px` complaint from §4 — this is a formatting detail, not a units decision, and costs nothing to get right.

**Asset delivery — format-matched export, clipboard-by-default with a size-aware zip recommendation.** Confirmed approach: `SVG` export for vector/icon layers (via `exportAsync({format: 'SVG'})`), `PNG`/`JPG` for raster image fills (matching source format where detectable, defaulting to `PNG` for anything with transparency and `JPG` otherwise) — this directly addresses the "everything exported as blurry PNG" complaint from §4. For delivery: keep **copy-to-clipboard as the default, fast path** (Base64-inlined assets in the copied HTML/CSS), but compute a simple heuristic while generating — e.g. **total asset payload over a threshold (a few MB) or asset count over a small number (say 5-10 images)** — and when it's crossed, surface a clear UI recommendation ("This export includes N large images — download the `.zip` instead for a cleaner result") alongside a **Download .zip** button that writes separate asset files plus HTML/CSS referencing them by relative path. Both paths should always be available; the zip is a *recommendation*, not a hard gate, so you're never blocking the fast path you explicitly asked for.

**Tooling — Plugma (Vite-based), your call, decided here.** You said you don't have a strong preference but leaned toward "Vite-based workflow is more efficient" — that's a reasonable instinct and the decision: **use [Plugma](https://www.plugma.dev/)**. Concretely, this buys you: a real Vite dev server with hot-module-reload while iterating on the settings/code-viewer UI (rather than `create-figma-plugin`'s rebuild-and-reload-the-plugin-in-Figma cycle), framework freedom (plain TypeScript, or React/Preact/Svelte if the UI grows), and built-in release tooling (keeps GitHub releases and the Figma Community version in sync with one command) — useful once you're shipping updates regularly. Trade-off worth knowing going in: `create-figma-plugin` ships pre-built Preact components already styled to match Figma's native UI look (buttons, dropdowns, inputs), which Plugma does not — with Plugma you'll either hand-roll Figma-styled UI components or pull in a separate component set. Given your UI is fairly small (a code viewer, a couple of buttons, maybe a few settings toggles), that's a minor, one-time cost, not an ongoing tax — so the Vite DX win is the right trade for this project.
